{ pkgs, config, ... }: let cfg = config.modules.programs.clipit; in {
  options = with pkgs.lib; {
    modules.programs.clipit = {
      enable = mkEnableOption "ClipIt script";
      username = mkOption {
        type = types.str;
        default = "admin";
      ...
      ziplineTokenFile = mkOption {
        type = types.pathInStore;
        description = "Path to a sops-encrypted file containing a Zipline token.";
      };
      cloudflareIdFile = mkOption {
        type = types.pathInStore;
        description = "Path to a sops-encrypted file containing a Cloudflare ID.";
      };
      cloudflareStreamTokenFile = mkOption {
        type = types.pathInStore;
        description = "Path to a sops-encrypted file containing a Cloudflare Stream token.";
      };
    };
  };
  config = pkgs.lib.mkIf cfg.enable {
    sops.secrets.zipline = {
      sopsFile = cfg.ziplineTokenFile;
      key = "";
      mode = "0440";
      format = "binary";
      owner = cfg.username;
    ...
    sops.secrets."cloudflare/token" = {
      sopsFile = cfg.cloudflareStreamTokenFile;
      key = "";
      mode = "0440";
      format = "binary";
      owner = cfg.username;
