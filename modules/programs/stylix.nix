...
in
{
  options = with pkgs.lib; {
    modules.programs.stylix = {
      ...
      wallpaperImageFile = mkOption {
        type = types.pathInStore;
        default = null;
        description = "Path in store to background/wallpaper image. Required by Stylix.";
      };
    };
  };
  config = pkgs.lib.mkIf cfg.enable {
    home-manager.users.${cfg.username} = {
      ...
      stylix = {
        ...
        autoEnable = true;
        polarity = "dark";
        image = cfg.wallpaperImageFile; # ../../assets/romb-3840x2160.png;
